...
for it in range(iterations):
    ...
    while True:
        result = problem.solve(verbose=False, solver='ECOS', max_iters=200)
        
        print("Result:", result)
        print("Status:", problem.prob.status)
...
# Specify the relative path to the 'output/trajectory' folder
trajectory_path = os.path.join(base_dir, 'output', 'trajectory')
...
save_arrays(trajectory_path, {'X': all_X, 'U': all_U, 'sigma': all_sigma})
